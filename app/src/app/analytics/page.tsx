...
export default function AnalyticsPage() {
  ...
  const totalBrowsers = data.devices.browsers.reduce((sum, b) => sum + b.count, 0);
  const totalOS = data.devices.os.reduce((sum, o) => sum + o.count, 0);
  const totalScreens = data.devices.screenResolutions.reduce((sum, s) => sum + s.count, 0);
  const totalLanguages = data.traffic.languages.reduce((sum, l) => sum + l.count, 0);
  const totalReferrers = data.traffic.referrers.reduce((sum, r) => sum + r.count, 0);
  const maxHourlyTraffic = Math.max(...data.traffic.hourlyPattern.map(h => h.count), 1);
  const darkModeCount = data.traffic.darkModeUsers.find(d => d._id === true)?.count || 0;
  const lightModeCount = data.traffic.darkModeUsers.find(d => d._id === false)?.count || 0;
  const totalModeUsers = darkModeCount + lightModeCount;

  const weekTrend = calculateTrend(data.overview.weekViews, data.overview.monthViews / 4);
  const todayTrend = calculateTrend(data.overview.todayViews, data.overview.weekViews / 7);

  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto py-8 space-y-8 max-w-[1400px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-blue-500/10 to-blue-600/5">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{data.overview.totalViews.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground mt-1">All time page views</p>
            </CardContent>
          </Card>

          <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-purple-500/10 to-purple-600/5">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{data.overview.uniqueVisitors.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground mt-1">Individual users tracked</p>
            </CardContent>
          </Card>

          <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-green-500/10 to-green-600/5">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{data.overview.todayViews}</div>
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className="text-xs text-muted-foreground">vs daily avg</span>
              </div>
            </CardContent>
          </Card>

          <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-orange-500/10 to-orange-600/5">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
            ...
            <CardContent>
              <div className="text-3xl font-bold">{formatDuration(Math.round(data.overview.avgTimeOnPage))}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="border-0 shadow-md hover:shadow-lg transition-all">
            ...
            <CardContent>
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className={`text-xs ${weekTrend.isPositive ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <Card className="border-0 shadow-md hover:shadow-lg transition-all">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatLoadTime(data.overview.avgPageLoadTime)}</div>
            ...
            <CardContent>
              <div className="text-sm space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono font-medium text-green-600">{formatLoadTime(data.overview.minPageLoadTime)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">Max</span>
                  <span className="font-mono font-medium text-red-600">{formatLoadTime(data.overview.maxPageLoadTime)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="border-0 shadow-lg">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl font-semibold">Top Pages</CardTitle>
          ...
          <CardContent>
            <div className="space-y-6">
              {data.topPages.map((page, index) => {
                const percentage = (page.views / data.overview.totalViews) * 100;
                return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>User devices accessing your site</CardDescription>
                </div>
                    ...
                    <div key={device._id} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-sm text-muted-foreground ml-2">({device.count})</span>
                        </div>
                      </div>
                      <div className="relative h-3 bg-muted rounded-full overflow-hidden">
                        <div 
                          className={`absolute inset-y-0 left-0 rounded-full transition-all duration-500
                            ${device._id === 'Desktop' ? 'bg-blue-500' : 
                              device._id === 'Mobile' ? 'bg-green-500' : 'bg-purple-500'}`}
                          style={{ width: `${percentage}%` }}
                        />
                      </div>
                    </div>
          ...
          <Card className="border-0 shadow-lg">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl font-semibold">Browser Usage</CardTitle>
            ...
            <CardContent>
              <div className="space-y-4">
                {data.devices.browsers.slice(0, 5).map((browser, index) => {
                  ...
                  return (
                    <div key={`${browser._id.name}-${browser._id.version}`} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {getBrowserIcon(browser._id.name)}
                          <span className="font-medium">{browser._id.name} {browser._id.version}</span>
                        </div>
                        <div className="text-right">
                          <span className="font-semibold">{percentage.toFixed(1)}%</span>
                          <span className="text-sm text-muted-foreground ml-2">({browser.count})</span>
                        </div>
                      </div>
                      <div className="relative h-3 bg-muted rounded-full overflow-hidden">
                        <div 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <Card className="border-0 shadow-lg">
            ...
            <CardContent className="space-y-3">
              {data.devices.os.slice(0, 5).map((os) => {
                ...
                return (
                  <div key={`${os._id.name}-${os._id.version}`} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    <span className="text-sm font-medium">{os._id.name} {os._id.version}</span>
          ...
          <Card className="border-0 shadow-lg">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg font-semibold">Screen Resolutions</CardTitle>
            ...
            <CardContent className="space-y-3">
              {data.devices.screenResolutions.slice(0, 5).map((res) => {
                ...
                return (
                  <div key={res._id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
          ...
          <Card className="border-0 shadow-lg">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg font-semibold">Theme Preference</CardTitle>
                <Moon className="h-4 w-4 text-muted-foreground" />
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-gray-900/5 dark:bg-gray-100/5 rounded-lg">
                  <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold">
                      {totalModeUsers > 0 ? `${((darkModeCount / totalModeUsers) * 100).toFixed(0)}%` : "0%"}
                    </p>
                    <p className="text-xs text-muted-foreground">{darkModeCount} users</p>
                  </div>
                </div>
                <div className="flex items-center justify-between p-4 bg-yellow-500/5 rounded-lg">
                  <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold">
                      {totalModeUsers > 0 ? `${((lightModeCount / totalModeUsers) * 100).toFixed(0)}%` : "0%"}
                    </p>
                    <p className="text-xs text-muted-foreground">{lightModeCount} users</p>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              {data.traffic.countries.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  {data.traffic.countries.slice(0, 10).map((country, index) => (
                    <div key={country._id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted/70 transition-colors">
                      <div className="flex items-center gap-2">
              ...
              {data.traffic.referrers.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.traffic.referrers.slice(0, 6).map((ref) => {
                    ...
                    return (
                      <div key={ref._id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted/70 transition-colors">
                        <div className="flex items-center gap-3">
                          {favicon && (
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {data.traffic.languages.map((lang) => {
                ...
                return (
                  <div key={lang._id} className="p-4 bg-muted/50 rounded-lg text-center hover:bg-muted/70 transition-colors">
                    <p className="font-mono font-semibold text-lg">{lang._id}</p>
                    <p className="text-sm text-muted-foreground">{percentage.toFixed(1)}%</p>
                    <p className="text-xs text-muted-foreground">{lang.count} users</p>
                  </div>
                );
              })}
            </div>
        ...
        <Card className="border-0 shadow-lg">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl font-semibold">Hourly Traffic Pattern</CardTitle>
                <CardDescription>Last 7 days activity by hour (GMT)</CardDescription>
              </div>
          ...
          <CardContent>
            <div className="space-y-1">
              {Array.from({ length: 24 }, (_, hour) => {
                const hourData = data.traffic.hourlyPattern.find(h => h._id === hour);
                ...
                return (
                  <div key={hour} className="flex items-center gap-3 group">
                    <span className="text-xs font-mono w-10 text-muted-foreground group-hover:text-foreground transition-colors">
                      {hour.toString().padStart(2, "0")}:00
                    </span>
                    <div className="flex-1 relative h-6 bg-muted/50 rounded overflow-hidden group-hover:bg-muted/70 transition-colors">
                      <div
                        className={`absolute inset-y-0 left-0 transition-all duration-500 rounded
                          ${isPeak ? 'bg-gradient-to-r from-red-500 to-orange-500' : 
              ...
              <Table>
                ...
                <TableBody>
                  {data.recentVisits.map((visit, index) => {
                    ...
                    return (
                      <TableRow key={`${visit.timestamp}-${index}`} className="hover:bg-muted/50 transition-colors">
                        <TableCell>
                          <div>
                            <p className="font-medium text-sm">{visitDate.toLocaleDateString()}</p>
                            <p className="text-xs text-muted-foreground">{visitDate.toLocaleTimeString()} • {timeAgo}</p>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <CheckCircle2 className="h-3 w-3 text-green-500" />
                            <span className="font-medium">{visit.path === "/" ? "Homepage" : visit.path}</span>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-sm">{visit.browser.name} {visit.browser.version}</span>
                          </div>
                        </TableCell>
                      </TableRow>
                    );
